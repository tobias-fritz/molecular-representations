...
class Molecule:
    ...
    def read_file(self, fname: str) -> None:
        ...
        if fname.endswith('.pdb'):
            ...
            self._read_mol2(fname)
        elif fname.endswith('.mol'):
            self._read_mol(fname)
    ...
    def _read_gro(self, fname: str) -> None:
        """Read atomic data from GRO file"""
        with open(fname, 'r') as ff:
            lines = ff.readlines()
        ...
        self.atoms = pd.DataFrame(gro).set_index('serial')
    ...
    def _read_mol2(self, fname: str) -> None:
        """Read atomic data from MOL2 file"""
        with open(fname, 'r') as ff:
            lines = ff.readlines()
        ...
        self.atoms = pd.DataFrame(mol2)

    def _read_mol(self, fname: str) -> None:
        """Read atomic data from MOL file"""
        with open(fname, 'r') as ff:
            lines = ff.readlines()
    ...
    def write_file(self, fname: str) -> None:
        ...
        if fname.endswith('.pdb'):
            ...
            self._write_mol2(fname)
        elif fname.endswith('.mol'):
            self._write_mol(fname)
        elif fname.endswith('.psf'):
    ...
    def _write_gro(self, fname: str) -> None:
        ...
        with open(fname, 'w') as ff:
            ...
            for idx, atom in self.atoms.iterrows():
                ff.write(f"{atom['resid']:5}{atom['resname']:5}{atom['atom_name']:5}{idx:5}{atom['x']:8.3f}{atom['y']:8.3f}{atom['z']:8.3f}\n")
    ...
    def _write_mol2(self, fname: str) -> None:
        """Write atomic data to MOL2 file"""
        with open(fname, 'w') as ff:
            ff.write("@<TRIPOS>MOLECULE\n")
            ff.write(f"{self.name}\n")
            ff.write(f"{len(self.atoms)} 0 0\n")
            ff.write("SMALL\n")
            ff.write("NO_CHARGES\n")
            ff.write("\n")
            ff.write("@<TRIPOS>ATOM\n")
            for idx, atom in self.atoms.iterrows():
                ff.write(f"{idx} {atom['atom_name']} {atom['x']} {atom['y']} {atom['z']} {atom['atom_type']} {atom['residue']} {atom['resid']} {atom['charge']}\n")

    def _write_mol(self, fname: str) -> None:
        """Write atomic data to MOL file"""
        with open(fname, 'w') as ff:
            ff.write(f"{len(self.atoms)}\n")
            for idx, atom in self.atoms.iterrows():
                ff.write(f"{atom['x']} {atom['y']} {atom['z']} {atom['atom_name']}\n")
